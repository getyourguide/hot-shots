...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (this.socket && !options.isChild) {
...
Client.prototype._close = function (callback) {
  ...
  const closeErrorHandler = (err) => {
    const error = new Error(`Error closing hot-shots socket: ${err}`);
    if (handledError) {
...
Client.prototype.handleError = function (err) {
  if (this.errorHandler) {
    ...
    console.error('hot-shots:', String(err));
  }
};
...
function udsErrorHandler(client, lastSocketCreateTime) {
  ...
  if (client.errorHandler) {
    client.socket.removeListener('error', client.handleError);
  }
  ...
  if (newSocket) {
    ...
    client.handleError(new Error('Could not replace UDS connection with new socket'));
    return;
  }
  client.socket.on('error', client.handleError);
}
